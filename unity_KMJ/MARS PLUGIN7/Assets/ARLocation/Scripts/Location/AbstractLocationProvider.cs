...
namespace ARLocation
{
    ...
    public abstract class AbstractLocationProvider : ILocationProvider
    {
        ...
        public double distanceFromStartPoint
        {
            get
            {
                return LocationReading.HorizontalDistance(firstLocation, currentLocation);
            }
        }
        ...
        public event CompassUpdateDelegate CompassUpdatedRaw;
        ...
        protected void EmitLocationUpdated()
        {
            ...
            {
                LocationUpdated(currentLocation, lastLocation);
            }
        }

        protected void EmitLocationUpdatedRaw()
        {
            if (LocationUpdatedRaw != null)
            {
                LocationUpdatedRaw(currentLocationRaw, lastLocationRaw);
            }
        }
        ...
        protected bool ShouldUpdateLocation(LocationReading newLocation)
        {
            ...
            if ((options.MaxDistanceBetweenUpdates > 0) && (LocationReading.HorizontalDistance(currentLocationRaw, newLocation) > options.MaxDistanceBetweenUpdates))
            {
                return false;
            }
            ...
            if ((newLocation.accuracy > options.MaxAccuracyRadius) && (options.MaxAccuracyRadius > 0))
            {
                return false;
            }
